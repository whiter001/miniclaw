...
MiniClaw 的 Weixin 接入采用 OpenClaw 风格的后端协议。当前实现不是直接调用微信官方 SDK，而是提供一个本地 HTTP 后端，让微信通道把消息交给 MiniClaw，再把回复送回去。
...
推荐优先传 `msg` 包裹，因为它最接近后续真实 OpenClaw / Weixin 消息适配。
...
这个二维码不是 MiniClaw 生成的，而是 OpenClaw 的 Weixin 登录流程生成的。
...
`miniclaw channels login --channel openclaw-weixin` 会直接把登录流程交给本机的 `openclaw`。二维码会显示在终端里，用微信扫码并确认授权，登录凭据会自动保存到本地。

如果你的 OpenClaw 版本太旧，先检查：

```bash
openclaw --version
```

如果版本不满足插件要求，就安装 legacy 版本：
...
其中 `ingest` 是本地桥接入口，不是 OpenClaw 标准协议端点，但它能把“收到消息”这一步接进 MiniClaw 的自动回复链路。
